...
- **Frontend**: React with Vite, Tailwind CSS (v3 + Flowbite plugin), Recharts, and jwt-decode
...
│   │   └── productController.js # Product CRUD (code immutable, ProductStatus ATIVO/INDISPONIVEL/INATIVO, doterraUrl, price history, soft-delete; GET list with search/sort/pagination + pageSize=all + ?available=true)
│   ├── utils/
...
│   ├── productLoader.test.js # 18 CSV parsing + catalog loader (idempotent diff, price history, retroactive date, dry-run, INDISPONIVEL preservation) tests
│   └── products.test.js # 38 Product CRUD tests (create/duplicate 409, list+search+sort+pagination incl. pageSize=all, status filters incl. available=true, doterraUrl validation, update w/ immutable code + price history, soft-delete)
├── frontend/
...
✅ Product + ProductPrice entities — global dōTERRA catalog with price history (`validFrom`/`validTo` interval), `pv` as Decimal(10,2), `ProductStatus` enum (ATIVO/INDISPONIVEL/INATIVO) and nullable `doterraUrl` (product page link)
...
✅ Product CRUD controller + routes (`src/controllers/productController.js`, `src/routes/productRoutes.js`) at `/api/products` — GET list (`?status=` single/multiple, `?available=true` for ATIVO+INDISPONIVEL, legacy `?active=true/false` alias, `q` partial name/code search, `sortBy` name/code/regularPrice/memberPrice/pv + `sortDir`, `page`/`pageSize` pagination returning `{ data, pagination }`, plus `pageSize=all` to return the full list), GET by id, POST create (409 on duplicate code, optional validated `doterraUrl`), PUT update (**code immutable** via update Zod schema; `status` enum; `doterraUrl` cleared with `null`; price changes close current `validTo` and open a new price record), DELETE soft-delete (`status = INATIVO`)
✅ Orders validate products against `status IN [ATIVO, INDISPONIVEL]` — INATIVO products are rejected on create/update; INDISPONIVEL products remain selectable in new/edit orders
✅ ProductsPage component that loads the whole catalog once (`/products?pageSize=all`) and applies search (name/code), sort dropdown and ATIVO/INDISPONIVEL/INATIVO status filter 100% client-side via `useMemo`; infinite scroll (IntersectionObserver sentinel) slices the in-memory list 20 rows at a time with no extra API calls; create/edit modals (code field disabled on edit with "O código não pode ser alterado", optional `doterraUrl` field, 3-state status select), "Site" external-link column, and inline status dropdown actions (`src/pages/ProductsPage.jsx`)
✅ Product CRUD test suite: 38 backend tests (create/duplicate 409/auth 401-403, list + status/available filter + search + sort + pagination incl. `pageSize=all`, get by id, update metadata/code immutability/status/price history, `doterraUrl` validation, soft-delete) + 29 frontend tests
...
✅ Tailwind CSS setup with PostCSS and Vite integration (with the Flowbite plugin — `flowbite@^2.5.2` in `frontend/package.json`, registered as `flowbitePlugin` in `tailwind.config.js` via `import flowbitePlugin from 'flowbite/plugin.js'`, and `./node_modules/flowbite/**/*.js` added to `content`)
✅ Responsive tables (Flowbite `data-label` card pattern) — the Clientes, Pedidos, Recebíveis, Produtos and "Resumo por Ano" tables no longer use `overflow-x-auto`. Each `<td>` carries a `data-label="Coluna"` attribute and the classes `block md:table-cell` + `before:content-[attr(data-label)] before:block before:text-xs before:font-semibold before:text-gray-500 before:mb-1 before:uppercase md:before:hidden` (plus `dark:before:text-gray-400`); each `<tr>` is `block md:table-row border ... rounded-lg shadow-sm mb-3 md:mb-0`; `<thead>` is `hidden md:table-header-group`; `<tbody>` is `block md:table-row-group` and `<table>` is `block md:table`. Below `md` each row renders as a stacked card with the column name as a label above each value (no horizontal scrollbar at any width); at `md+` the table is identical to before. All 282 frontend tests pass unchanged (DOM structure preserved)
...
- **Phase 39**: ✅ Product status "Indisponível" + dōTERRA site link — the `Product.active` boolean was replaced by a `ProductStatus` enum (**ATIVO / INDISPONIVEL / INATIVO**) and a new nullable `doterraUrl VARCHAR(2048)` field was added (migration `20260812160000_add_product_status_and_url`, applied via `migrate deploy`, `active=true→ATIVO` / `active=false→INATIVO`, no data loss). The Products screen now shows a **3-state status filter** and **badges** (Ativo green / Indisponível amber with `AlertCircle` / Inativo gray), an optional **"URL do produto no site da dōTERRA"** field in both create/edit modals (validated as URL), a **"Site"** external-link column, and an **inline status dropdown** replacing the old Desativar/Ativar button. Orders behavior: `validateProducts` now accepts products with `status IN [ATIVO, INDISPONIVEL]` (rejects INATIVO with `'One or more products are inactive or do not exist'`); the Orders page fetches the combobox list via `/products?available=true` (ATIVO + INDISPONIVEL), so INDISPONIVEL products remain selectable in new/edited orders while INATIVO products never appear as selectable options — but an order already containing an INATIVO product still shows its name/code in the edit form (via `item.product` fallback in the combobox). The catalog loader only deactivates products with `status = ATIVO` absent from the CSV, and preserves manual INDISPONIVEL status. 10 new backend tests + 9 new frontend tests. **168 backend tests, 282 frontend tests, 450 total tests.**
- **Phase 40**: ✅ Responsive lists with the Flowbite `data-label` pattern — adopted **Flowbite** (`flowbite@^2.5.2`, MIT, Tailwind v3-compatible) purely as a Tailwind plugin (`import flowbitePlugin from 'flowbite/plugin.js'` + `./node_modules/flowbite/**/*.js` in `content`) with **no extra JS**. The Clientes, Pedidos, Recebíveis, Produtos and Dashboard "Resumo por Ano" tables dropped `overflow-x-auto`; on `<md` each row renders as a stacked card (rounded border + shadow) with each column shown as a small uppercase label above its value (`data-label` + `before:content-[attr(data-label)]`), and on `md+` the tables are identical to before. All 39 `<td>` cells were migrated (8 People + 10 Orders + 9 Receivables + 9 Products + 3 yearly). `whitespace-nowrap` is now `md:whitespace-nowrap` (mobile values may wrap); right/center-aligned columns become left-aligned on mobile. No tests changed — all 282 frontend tests pass unchanged (DOM structure `<table>/<tr>/<td>` is preserved; only display classes/attributes changed). **168 backend tests, 282 frontend tests, 450 total tests.**
