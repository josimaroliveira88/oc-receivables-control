...
✅ Frontend validation: rejects only negative amounts; rejects zero against a person with chargeable items ("Valor deve ser maior que zero"); asks for `window.confirm` confirmation when the amount exceeds the pending balance (overpayment)
