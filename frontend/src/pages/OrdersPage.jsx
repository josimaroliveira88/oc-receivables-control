  ...
  const handleEditOrder = async (order) => {
    ...
    setItems(
      order.items.map((item) => ({
        ...
        chargedValue:
          item.chargedValue != null
            ? parseFloat(item.chargedValue).toString()
            : '',
        personId: item.personId || '',
        ...
        memberPrice:
          item.memberPrice != null
            ? parseFloat(item.memberPrice).toString()
            : '',
                  ...
                  {orders.map((order) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {paymentTypeBadge(order.paymentType)}
                        </td>
                        <td
                          data-label="Valor (R$)"
                          className="block lg:table-cell px-3 lg:px-6 py-2 lg:py-4 lg:whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 before:content-[attr(data-label)] before:block before:text-xs before:font-semibold before:text-gray-500 dark:before:text-gray-400 before:mb-1 before:uppercase lg:before:hidden"
            ...
            >
              ...
              <div className="mb-4">
                ...
                {items.map((item, index) => (
                  <div
                    key={item.id}
                    className="border border-gray-200 dark:border-gray-700 rounded-md p-4 mb-3"
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="md:col-span-3">
                        ...
                        <select
                          value={item.personId}
                          onChange={(e) =>
                            updateItemField(index, 'personId', e.target.value)
                          }
                      ...
                      <div className="md:col-span-3">
                        ...
                        <textarea
                          value={item.details}
                          onChange={(e) =>
                            updateItemField(index, 'details', e.target.value)
                          }
                          maxLength={500}
                          rows={2}
                          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors text-sm"
