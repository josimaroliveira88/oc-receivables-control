...
const ReceivablesPage = () => {
  ...
  const getDetailPersonItems = (personId) => {
    ...
    return (detailOrder.items || []).filter((item) => item.personId === personId);
  };

  const getDetailPersonPayments = (personId) => {
    if (!detailOrder) return [];
    return (detailOrder.payments || []).filter((payment) => payment.personId === personId);
  };
            ...
            <div className="px-6 py-4">
              <div className="mb-4 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 p-3">
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                  <div>
                    <dt className="text-xs text-gray-500 dark:text-gray-400">Número</dt>
                    <dd className="text-sm font-medium text-gray-900 dark:text-gray-100">{detailOrder.orderNumber}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 dark:text-gray-400">Data</dt>
                    <dd className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatDateBR(detailOrder.orderDate)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 dark:text-gray-400">Responsável</dt>
                    <dd className="text-sm font-medium text-gray-900 dark:text-gray-100">{detailOrder.accountOwner || '—'}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 dark:text-gray-400">Valor Total</dt>
                    <dd data-testid="details-summary-total" className="text-sm font-medium text-gray-900 dark:text-gray-100">{formatBRL(parseFloat(detailOrder.totalValue))}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 dark:text-gray-400">Valor Pendente</dt>
                    <dd data-testid="details-summary-pending" className="text-sm font-medium text-gray-900 dark:text-gray-100">
                      {formatBRL(Math.max(0, toCents(parseFloat(detailOrder.totalValue)) - (detailOrder.payments || []).reduce((sum, payment) => sum + toCents(parseFloat(payment.amount)), 0)) / 100)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 dark:text-gray-400">Descrição</dt>
                    <dd data-testid="details-summary-description" className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate" title={detailOrder.orderNotes || undefined}>
              ...
              {detailLoading ? (
                ...
                <p className="py-6 text-center text-sm text-gray-500 dark:text-gray-400">Nenhuma pessoa neste pedido</p>
              ) : (
                <div className="rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
                  {detailBalances.map((balance) => {
                    ...
                    const personPayments = getDetailPersonPayments(balance.personId);
                    return (
                      <div key={balance.personId} className="border-b last:border-b-0 border-gray-200 dark:border-gray-700">
                        <button
                          type="button"
                          data-testid={`detail-person-${balance.personId}`}
                          aria-expanded={expanded}
                          onClick={() => setExpandedPersonId(expanded ? '' : balance.personId)}
                        ...
                        >
                          <span className="font-medium text-gray-900 dark:text-gray-100">{balance.personName}</span>
                          <span className="flex items-center gap-3 text-sm whitespace-nowrap">
                            <span className="text-gray-600 dark:text-gray-300">Total: {formatBRL(balance.itemTotal)}</span>
                            <span className={toCents(balance.pending) === 0 ? 'text-gray-400 dark:text-gray-500' : 'text-primary-700 dark:text-primary-400'}>
                        ...
                        {expanded && (
                          <div data-testid={`detail-panel-${balance.personId}`} className="px-3 pb-4 bg-gray-50 dark:bg-gray-900/30">
                            <h4 className="pt-3 mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Itens desta pessoa</h4>
                            {personItems.length === 0 ? (
                              <p className="text-sm text-gray-500 dark:text-gray-400">Nenhum item registrado para esta pessoa</p>
                            ) : (
                              <div className="rounded-md border border-gray-200 dark:border-gray-700 divide-y divide-gray-200 dark:divide-gray-700 overflow-hidden">
                                {personItems.map((item) => (
                                  <div key={item.id} className="px-3 py-2 bg-white dark:bg-gray-800">
                                    <div className="flex items-start justify-between gap-2">
                                      <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{item.product?.name || item.description || '—'}</span>
                                      <span className="text-sm font-semibold text-primary-700 dark:text-primary-400 whitespace-nowrap">{formatBRL(parseFloat(item.chargedValue))}</span>
                                    </div>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Detalhes: {item.details || '—'}</p>
                                  </div>
                                ))}
                              </div>
                            )}

                            <h4 className="mt-4 mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Pagamentos recebidos</h4>
                            {personPayments.length === 0 ? (
                              <p className="text-sm text-gray-500 dark:text-gray-400">Nenhum pagamento recebido</p>
                            ) : (
                              <div className="rounded-md border border-gray-200 dark:border-gray-700 divide-y divide-gray-200 dark:divide-gray-700 overflow-hidden">
                                {personPayments.map((payment) => (
                                  <div key={payment.id} className="px-3 py-2 bg-white dark:bg-gray-800">
                                    <div className="flex items-start justify-between gap-2">
                                      <span className="text-sm text-gray-900 dark:text-gray-100">{formatDateBR(payment.paidAt)}</span>
                                      <span className="text-sm font-semibold text-emerald-700 dark:text-emerald-400 whitespace-nowrap">{formatBRL(parseFloat(payment.amount))}</span>
                                    </div>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Observação: {payment.notes || '—'}</p>
                                  </div>
                                ))}
                              </div>
                            )}
