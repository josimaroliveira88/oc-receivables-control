    ...
    >
      {() => (
        <div className="px-6 py-4">
          ...
          {reconciliationBatchIds.length > 0 && (
            <div className="mb-4 flex flex-wrap items-center gap-2 rounded-md border border-info-soft bg-info-soft px-4 py-3">
              <p className="text-sm text-info-fg">
                Parcelas conciliadas com a fatura do cartão.
              </p>
              {reconciliationBatchIds.map((batchId) => (
                <button
                  key={batchId}
                  type="button"
                  data-testid={`bill-undo-batch-${batchId}`}
                  onClick={() => onUndoBatch(batchId)}
                  className="inline-flex items-center gap-1.5 rounded-md border border-line bg-surface px-3 py-1.5 text-sm font-medium text-ink-soft transition-colors hover:bg-elevated hover:text-ink"
                >
