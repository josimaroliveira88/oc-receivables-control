...
- Vite
- Tailwind CSS
- Flowbite (Tailwind plugin only, v2.5.2, for the responsive `data-label` table pattern)
- Recharts
- SheetJS (xlsx)
- lucide-react
- jwt-decode
...
✅ Comprehensive test coverage (319 frontend tests + 168 backend tests)
...
✅ Product status "Indisponível" + dōTERRA site link — `Product.active: Boolean` was replaced by a **`ProductStatus` enum (ATIVO / INDISPONIVEL / INATIVO)** plus a nullable **`doterraUrl VARCHAR(2048)`** field (migration `20260812160000_add_product_status_and_url`, applied via `migrate deploy`, no data loss). The Products screen now has a **3-state status filter and badges** (Ativo green / Indisponível amber with `AlertCircle` / Inativo gray), an optional **"URL do produto no site da dōTERRA"** field in both create/edit modals (URL-validated), a **Site** external-link column, and an **inline status dropdown** replacing the old Desativar/Ativar button. Orders: `validateProducts` accepts `status IN [ATIVO, INDISPONIVEL]` (rejects INATIVO with `'One or more products are inactive or do not exist'`); the Orders page loads its combobox list via `/products?available=true` (ATIVO + INDISPONIVEL), so INDISPONIVEL products stay selectable in new/edited orders while INATIVO products never appear as options — but an order already referencing an INATIVO product still shows its name/code in the edit form (via a `selectedName`/`selectedCode` fallback fed by `item.product`). The catalog loader only deactivates `status = ATIVO` products absent from the CSV and preserves manual INDISPONIVEL. 10 new backend tests + 9 new frontend tests.
✅ Responsive lists via Flowbite `data-label` pattern — adopted **Flowbite** (`flowbite@^2.5.2`, MIT, Tailwind v3-compatible) as a **pure Tailwind plugin** (`import flowbitePlugin from 'flowbite/plugin.js'` + `./node_modules/flowbite/**/*.js` in `content`; no extra JS). Applied the responsive table pattern to **all five lists** (Clientes, Pedidos, Recebíveis, Produtos and Dashboard "Resumo por Ano"), eliminating every `overflow-x-auto` and thus **all horizontal scrollbars**: `<table>` → `block md:table`, `<thead>` → `hidden md:table-header-group`, `<tbody>` → `block md:table-row-group`, `<tr>` → `block md:table-row border ... rounded-lg shadow-sm mb-3 md:mb-0`, each `<td>` → `block md:table-cell` + `data-label="Coluna"` + `before:content-[attr(data-label)] before:block before:text-xs before:font-semibold before:text-gray-500 dark:before:text-gray-400 before:mb-1 before:uppercase md:before:hidden`. Below `md` each row is a stacked card with an uppercase column label above every value (all columns still visible); at `md+` the tables are identical to before. `whitespace-nowrap` became `md:whitespace-nowrap`; right/center-aligned columns become left-aligned on mobile. All 282 frontend tests pass **unchanged** (DOM `<table>/<tr>/<td>` semantics preserved for accessibility and screen readers).
✅ Action menu dropdown (kebab) on the Orders `Ações` column — the inline "Editar" / "Excluir" text buttons were replaced by a single Flowbite-style kebab dropdown built on a reusable in-house React component (`frontend/src/components/ActionMenu.jsx`, ~80 lines, no new deps). The trigger is a `MoreVertical` lucide-react glyph inside `<button type="button" aria-haspopup="menu" aria-expanded={open} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 focus:ring-2 focus:ring-primary-500 transition-colors">`; clicking it opens a `<div role="menu" className="absolute right-0 mt-2 z-[80] w-44 ... divide-y ... rounded-lg shadow-lg">` anchored `bottom-end` to the trigger, listing one `<button role="menuitem">` per action (`icon + label`). Variants: `default` (`text-gray-700 dark:text-gray-200 hover:bg-gray-100`) and `danger` (`text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20`) — Excluir is the danger item. Click-outside via a `fixed inset-0 z-[70]` backdrop; `Escape` closes; items call `e.stopPropagation()` so row-level handlers stay clean. Z-index follows AGENTS.md lição #18: modais `z-[60]` · `ConfirmDialog` + backdrop `z-[70]` · menu panel `z-[80]`. The order `handleEditOrder(order)` / `handleDeleteOrder(order.id)` signatures are **preserved** — no business-logic changes. A migration playbook (`docs/ACTION_MENU_REFACTORING_GUIDE.md`) documents the API, recipe, helpers and acceptance checklist for replicating the same UX in **PeoplePage**, **ReceivablesPage** and **ProductsPage`. 7 new TDD tests added; 9 existing Edit/Delete tests updated via `openOrderActionsMenu` + `clickOrderAction` helpers. **289 frontend tests, 168 backend tests, 457 total, zero regressions.**
✅ Action menu dropdown (kebab) on the Clientes and Produtos `Ações` columns — following the `docs/ACTION_MENU_REFACTORING_GUIDE.md` playbook, the inline "Editar" / "Excluir" text buttons in **PeoplePage.jsx** were replaced by `<ActionMenu actions={[Editar (Pencil), Excluir (Trash, danger)]} ariaLabel="Ações do cliente" testIdPrefix="client-actions-${id}" />`, and the "Editar" text button in **ProductsPage.jsx** was replaced by `<ActionMenu actions={[Editar (Pencil)]} ariaLabel="Ações do produto" testIdPrefix="product-actions-${id}" />` (the inline status `<select>` is preserved alongside the kebab, inside a `flex items-center justify-end gap-2` wrapper — no business-logic changes; `openEditModal(product)` / `handleStatusChange(product, newStatus)` signatures intact). ReceivablesPage was intentionally skipped — its `Ações` column has only the `Registrar Pagamento` / `Pago` button (no Editar/Excluir). `relative` was added to the `<td>` so the absolutely-positioned menu anchors correctly. 7 new TDD tests per screen (trigger-per-row, menu-hidden-until-click, item labels, danger styling on Excluir, a11y semantics `aria-haspopup`/`aria-expanded`/`role="menu"`/`role="menuitem"`, backdrop-click-to-close, Escape-to-close); 5 existing Edit/Delete tests in PeoplePage + 3 in ProductsPage rewritten via `openClientActionsMenu`/`clickClientAction` and `openProductActionsMenu`/`clickProductAction` helpers. **303 frontend tests, 168 backend tests, 471 total, zero regressions.**
✅ Conditional "AÇÕES" column + kebab menu on the Controle de Recebíveis — `ReceivablesPage.jsx` now renders **only the discrete kebab** in the table cell, with actions inside its `bottom-end` dropdown. Orders with **pending > 0** receive the highlighted primary item **"Registrar Pagamento"** (`variant: 'primary'`, `bg-primary-600`, white text) plus **"Detalhar"**; zero-value brinde orders still pending settlement (`pendingCents === 0 && totalCents === 0 && status !== 'QUITADO'`) receive **"Dar baixa"** plus **"Detalhar"**; fully paid/overpaid orders (pending 0) receive only **"Detalhar"**. The disabled "Pago" text was removed. `ActionMenu.jsx` now supports the `primary` item variant in addition to `default` and `danger`, with fixed icon width and left-aligned labels so wrapped labels remain aligned. The detail callback remains an intentional stub (`console.log('Detalhar — pedido', order.id)`) — the detail modal/screen is deferred to a future phase. The **Valor Pendente** cell uses muted styling `text-gray-400 dark:text-gray-500` when `pendingCents === 0`. `testIdPrefix` = `receivable-actions-${order.id}`, `ariaLabel` = "Ações do pedido", `relative` on the `<td>`. No new dependencies; backend unchanged. Tests updated to open payment actions through the dropdown; 314 frontend tests, 168 backend tests, 482 total, zero regressions.

✅ Phase 44 detalhamento de recebíveis — o item "Detalhar" agora abre modal com o cabeçalho idêntico ao de Registrar Pagamento, totais por pessoa obtidos de `/balance` e acordeão exclusivo com itens e pagamentos recebidos. **319 frontend tests, 168 backend tests, 487 total tests.**

### Test Results:
- **Backend Tests**: 168 passing (26 People + 46 Orders + 31 Payments + 6 Dashboard + 4 Auth + 18 ProductLoader + 38 Products)
- **Frontend Tests**: 314 passing (30 PeoplePage + 64 OrdersPage + 62 ReceivablesPage + 26 DashboardPage + 32 exportExcel + 10 api + 20 RegisterPage + 10 LoginPage + 6 Header + 11 MobileDrawer + 36 ProductsPage + 7 ThemeContext)
- **Total**: 487 tests passing with zero regressions
...
24 critical lessons learned documented in AGENTS.md (see "Lessons Learned / Pitfalls to Avoid") to guide future development:
...
24. Flowbite plugin must be imported with the `.js` extension in ESM `tailwind.config.js`
...
6. **Run Full Test Suite**: Verify all 450 tests pass with zero regressions
...
**Problem**: The frontend `tailwind.config.js` is an ES module (`export default`), but Flowbite's docs only show the CommonJS form (`require('flowbite/plugin')`). Writing `import flowbitePlugin from 'flowbite/plugin';` fails at build/load time with:
```
Error [ERR_MODULE_NOT_FOUND]: Cannot find module '.../node_modules/flowbite/plugin'
```
because Flowbite's `package.json` has **no `exports` field**, so Node's ESM resolver requires the explicit filename and cannot infer `plugin` → `plugin.js`.

**Fix**: Always import with the extension:
```js
// tailwind.config.js (ESM)
import flowbitePlugin from 'flowbite/plugin.js';
```
Also add `./node_modules/flowbite/**/*.js` to the `content` array so Tailwind scans Flowbite's own markup. The plugin itself is optional — the responsive `data-label` card pattern is pure Tailwind utility classes (`block md:table-cell`, `before:content-[attr(data-label)]`, `md:before:hidden`, etc.) and works with or without the plugin; Flowbite was adopted here as a design-system convention and future component source.

**Note on the responsive pattern**: the CSS-only mobile-card layout uses `before:content-[attr(data-label)]` arbitrary values and `md:before:hidden` to hide the label at `md+`. Verify generated CSS with `npx tailwindcss -i ./src/index.css -o /tmp/out.css --minify` and grep for `data-label` + `table-cell`. The `data-label` labels render only visually via `::before`; the DOM keeps real `<table>/<tr>/<td>` elements, so screen readers and tests that select `tbody tr` keep working.
